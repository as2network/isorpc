...
import { VMType } from './proto/as2_pb'
import {
  EvmTxReceipt,
  EvmTxObject,
  EthBlockInfo,
...
export enum ClientEvent {
  ...
  /**
   * Exclusively used by as2ng-provider
   */
  EVMEvent = 'evmEvent',
  /**
  ...
  Error = 'error',
  /**
   * Emitted when a connection is established to the networkChain.
   * Listener will receive INetEventArgs.
   */
  Connected = 'connected',
  /**
   * Emitted when a connection with the networkChain is closed.
   * Listener will receive INetEventArgs.
   */
...
/**
 * Writes to & reads from a AS2ng networkChain.
 *
 * The client can listen to events emitted by smart contracts running on a networkChain,
...
export class Client extends EventEmitter {
  ...
  /**
   * Indicates whether the client is configured to use the /eth endpoint on AS2ng nodes.
  ...
  /** Middleware to apply to transactions before they are transmitted to the networkChain. */
  ...
  /**
   * Constructs a new client to read & write data from/to a AS2ng networkChain via web sockets.
   * @param chainId networkChain identifier.
   * @param writeUrl Host & port to send txs, specified as "<protocol>://<host>:<port>".
   * @param readUrl Host & port of the networkChain read/query interface, this should only be provided
   *                if it's not the same as `writeUrl`.
   */
  constructor(chainId: string, writeUrl: string, readUrl?: string)
  /**
   * Constructs a new client to read & write data from/to a AS2ng networkChain.
   * @param chainId networkChain identifier.
   * @param writeClient RPC client to use to send txs to the networkChain.
   * @param readClient RPC client to use to query the networkChain and listen to networkChain events, this
   ...
   * Once disconnected the client can no longer be used to interact with the networkChain.
   */
  disconnect() {
  ...
  /**
   * Commits a transaction to the networkChain.
   *
   * Consider using Contract.callAsync() instead.
   *
   * @param tx Transaction to commit.
   * @param opts Options object.
   * @param opts.middleware Middleware to apply before sending the tx to the networkChain, setting this
  ...
  /**
   * Sends a Web3 JSON-RPC message to the /eth endpoint on a AS2ng node.
   ...
   *
   * @param contractName Name of a smart contract on a AS2ng networkChain.
